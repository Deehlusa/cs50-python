...
def get(msgid, default):
    # Fork (PT-BR parcial): qualquer falha ao traduzir uma entrada cai pro ingles,
    # em vez de quebrar o build inteiro. A traducao br oficial e incompleta.
    try:
        return _get(msgid, default)
    except Exception:
        return default


def _get(msgid, default):
    assert msgid
    assert default
    ...
    if result == msgid:
        # Fork (PT-BR parcial): a traducao br nao cobre tudo. Em vez de quebrar o
        # build com assert, cai pro texto padrao (ingles) quando faltar traducao.
        return default
