...
import {saveServerProgress} from "./book/serverSync";
...
const saveToLocalStoreMiddleware = store => next => async (action) => {
  ...
  next(action);
  const {user, editorContent, pages} = store.getState().book;
  ...
  // Espelha no SQLite do server.py (best-effort, debounced)
  saveServerProgress(pages, user, editorContent);
};
